...
setwd("C:\\Users\\Sdhiraj\\Documents\\My_R_space\\R_Scripts")
# reading the data
credit_data <- read.csv("C:\\Users\\Sdhiraj\\Documents\\My_R_space\\datasets\\credit_data.csv")
...
credit_data %>%
  ggplot(aes(x = factor(loan_status), y = annual_inc)) +
  geom_boxplot()
  
credit_data %>%
  ggplot(aes(x = factor(loan_status), y = loan_amnt)) +
  geom_boxplot()
...
summary(credit_data)
# not removing outliers from annual_inc and loan_amnt, because i want
# the model to work on all type of observations

# dealing with missing values

# deleting observations / imputing median / coarse classification
# here i am using coarse classification by using different bins
...
# checking range of predicted probabilities to check wheather the model distinguishes 
# good and bad customers properly
range(predictions)

# checking auc of model
library(pROC)
auc(credit_testing$loan_status, predictions)

# selecting a proper threshold / cut-off value

# pred_15 <- ifelse(predictions > 0.15, 1, 0)
# table(credit_testing$loan_status, pred_15)
# 
# pred_20 <- ifelse(predictions > 0.20, 1, 0)
...
cuttoff <- quantile(prediction_stepmodel, 0.8)
